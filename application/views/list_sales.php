								...
								<table id="example" cellspacing="0" width="100%" class="table table-bordered">
									<thead>
										<tr>
										    ...
										    <th width="14%"><?php echo $lang_grand_total; ?></th>
											
    
										    <th width="5%"><?php echo $lang_action; ?></th>
...
<?php
    ...
    if ($user_role == 1) {
        ...
        $orderResult = $this->db->query("SELECT * FROM orders WHERE ordered_datetime >= '$today_start' AND ordered_datetime <= '$today_end' AND outlet_id= '$user_outlet' AND created_user_id='$user_id' ORDER BY id DESC ");
    }
...
<div class="">
<h3 class="ciniki"><?php echo "Cinikin Yau "; ?> :N <?php echo number_format($alltotal, 2); ?> </h3>
</div>
									</tbody>
								</table>
							</div>
