	...
	$(document).on('ready', function() {
		...
		$("#pcodeBarcode").keyup(function (e) {
			if (e.keyCode == 13) {
				...
				row_count 				= parseFloat(row_count);
								...
								if(add_row_count == 1){
									...
									if(check_existing_pcode == 0){
										...
										for(var q = 1; q < add_row_count; q++){
											...
											exist_qty 			= parseFloat(exist_qty);
...
<div class="row">
	<div class="col-md-12">
		<input type="text" class="form-control" id="pcodeBarcode" style="border: 1px solid #373942;" placeholder="<?php echo $lang_scan_barcode; ?>" autofocus />
	</div>
</div>
							...
							<div class="row">
								...
								<div class="row">
									<div class="col-md-4" style="margin-top: 8px;">
										<div style="background-color: #c72a25; color: #FFF; text-align: center; font-weight: bold; border-radius: 4px; cursor: pointer; padding-top: 10px; padding-bottom: 10px; width: 100%;" onclick="window.location.reload()">
...
<div class="row" style="margin-left: 0px; margin-right: 0px;">
	<div class="col-md-12" style="padding-left: 15px; padding-right: 15px; padding-top: 10px;">
		<input type="text" class="form-control" style="border: 1px solid #3a3a3a; color: #010101;" placeholder="<?php echo $lang_search_product_by_namecode; ?>" onclick="this.value=''" id="searchProd"  />
	</div>
<!--
...
<script src="<?=base_url()?>assets/js/jquery-2.2.0.min.js"> type="text/javascript"></script>
                                    ...
                                    ?>	
										...
										<?php
                                            $allProdData = $this->Constant_model->getDataAll('pos_item', 'code', 'ASC');
                                            for ($ap = 0; $ap < count($allProdData); ++$ap) {
                                                $pcode = $allProdData[$ap]->code;
                                                $name = $allProdData[$ap]->name;
												$retail_price = $allProdData[$ap]->retail_price;
												$qty = $allProdData[$ap]->qty;
												...
												<?php
													...
													<?php
                                                        ...
                                                        if (($display_prod == '1') || ($display_prod == '3') ) {
															...
															?>
													<span id="proname"><?php echo $retail_price; ?><br/><?php echo $name; ?> <br/><?php echo $qty; ?></span>
													<?php
	        ...
	        if(card_numb.length == 0) {
				...
				document.getElementById("submit_btn").style.display = "none";
					...
					<div class="row" style="padding-top: 10px; padding-bottom: 10px;">
						...
						<div class="col-md-6">
							<input type="text" name="transfer_paid" id="transfer_paid" class="form-control" placeholder="0.00" style="border: 1px solid #3a3a3a; color: #010101;" <?php if ($keyboard == '1') {
...
<script>
			...
			var resultPrice = $.ajax({
					...
					success	: function(data) {
						...
						if(add_row_count == 1){
							...
							if(qty >= 1){
							var msgs 		= '<div class="row" id="item_row_'+add_row_count+'" style="margin: 0px; border-bottom: 1px solid #dddddd; padding-top: 5px; padding-bottom: 5px;"><div class="col-md-4" style="background-color: #4d9fe4; color: #FFF;">'+name+' <br />['+pcode+']</div><div class="col-md-4" style="text-align: center; font-size: 15px; color: #000;"><div class="row"><div class="col-md-3" style="padding-top:7px"><span onclick="minusDiv('+add_row_count+')" style="cursor:pointer;"><img src="<?=base_url()?>assets/img/minus_icon.png" /></span></div><div class="col-md-6" style="padding-left: 0px; padding-right: 0px;"><input type="text" id="display_qty_'+add_row_count+'" class="form-control" value="1" onClick="this.select();" oninput="manualQty(this.value, '+add_row_count+')" style="text-align:center;" /></div><div class="col-md-3" style="padding-left:0px; padding-top:7px;"><span onclick="plusDiv('+add_row_count+')" style="cursor:pointer;"><img src="<?=base_url()?>assets/img/plus_icon.png" /></span></div></div></div><div class="col-md-3">'+price+'</div><div class="col-md-1" style="font-weight: bold;"><span onclick="deleteDiv('+add_row_count+')" style="cursor:pointer;">x</span></div></div><input type="hidden" name="pcode_'+add_row_count+'" id="pcode_'+add_row_count+'" value="'+pcode+'" /><input type="text" name="price_'+add_row_count+'" onClick="this.select();" id="price_'+add_row_count+'" value="'+price+'" /><input type="hidden" name="qty_'+add_row_count+'" id="qty_'+add_row_count+'" value="1" />';
								logDiv.append( msgs + "" );
	...
	function deleteDiv(ele){
		...
		document.getElementById("price_"+ele).style.display = "none";
	...
	function calculatePaidAmt(ele){	
		...
		var pay_minus_trans     = payable_amt - transfer_amt;
		
		
		if(paid_by == "6"){
			var change_amt 			= 0;
		} else {
			var change_amt 			= pay_minus_trans - ele;
		}
