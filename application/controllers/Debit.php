...
class Debit extends CI_Controller
{
    ...
    {	$new_payment = 0;
        ...
        $paymentDtaData = $this->Constant_model->getDataOneColumn('order_payments', 'id', $order_id);
        $ord_id = $paymentDtaData[0]->order_id;
		$payment = $paymentDtaData[0]->payment_amount;
		
		$ordDtaData = $this->Constant_model->getDataOneColumn('orders', 'id', $ord_id);
		$current_payment = $ordDtaData[0]->paid_amt;
		$cust_id = $ordDtaData[0]->customer_id;
		
		$new_payment = $current_payment - $payment;
		
		$upd_order_data 	= array(
					
					"payment_method_name"	=>	$pay_meth_name,
					"cheque_number"			=>	$cheque,
					"paid_amt"				=>	$new_payment,
					"updated_user_id"		=>	$us_id,
					"updated_datetime"		=>	$tm,
					"vt_status"				=>	$vt_status,
					"card_number"			=>	$addi_card_numb
			);
	        $this->Constant_model->updateData("orders", $upd_order_data, $ord_id);
			$this->Constant_model->deleteData('order_payments', $order_id);
			redirect(base_url().'customers/payment_history?cust_id'.$cust_id);
		   
        // Delete Order;
        
		
		
		unset($paymentDtaData);
		unset($ordDtaData);
