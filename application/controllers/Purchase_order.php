...
class Purchase_order extends CI_Controller
{
    ...
    public function __construct()
    {
		...
		$this->load->model('Return_purchaseorder_model');
	...
	// View Purchase Order;
    public function return_po_view()
    {
        $paginationData = $this->Constant_model->getDataOneColumn('site_setting', 'id', '1');
        $pagination_limit = $paginationData[0]->pagination;
        $setting_dateformat = $paginationData[0]->datetime_format;
        ...
        $config['total_rows'] = $this->Return_purchaseorder_model->record_po_count();
        $config['per_page'] = $pagination_limit;
        $config['uri_segment'] = 3;
        ...
        $data['results'] = $this->Return_purchaseorder_model->fetch_po_data($config['per_page'], $page);

        $data['links'] = $this->pagination->create_links();

        if ($page == 0) {
            $have_count = $this->Return_purchaseorder_model->record_po_count();
            ...
            $sh_text = "Showing $start_point to ".count($data['results']).' of '.$this->Return_purchaseorder_model->record_po_count().' entries';
        } else {
            $start_sh = $page + 1;
            $end_sh = $page + count($data['results']);
            $sh_text = "Showing $start_sh to $end_sh of ".$this->Return_purchaseorder_model->record_po_count().' entries';
        }
        ...
        $this->load->view('return_purchase_order', $data);
    }
    ...
    public function return_purchase_order()
    {
        ...
        $this->load->view('create_purchase_order_return', $data);
    }
	...
	public function return_viewpo()
    {
        ...
        $this->load->view('view_return_purchase_order', $data);
    }
    ...
    public function ReceiveItemsPO()
    {
		...
		$return_change = $this->input->post('return_change');
        ...
        for ($ex = 0; $ex < count($existItemData); ++$ex) {
            ...
            if (isset($_POST["receiveQty_$ex_item_id"]) && isset($_POST["receiveCost_$ex_item_id"])) {
				...
				$rec_price = $this->input->post("receivePrice_$ex_item_id");
    ...
    public function insertNewPO()
    {
	 ...
	 // Insert New Purchase Order;
    public function insertReturnPO()
    {
        $po_numb = strip_tags($this->input->post('po_number'));
        $outlet = strip_tags($this->input->post('outlet'));
        $supplier = strip_tags($this->input->post('supplier'));
        ...
        if (empty($po_numb)) {
            ...
            */
            ...
            if ($ckPORows > 0) {
                ...
                $po_id = $this->Constant_model->insertDataReturnLastId('return_purchases', $ins_po_data);
                ...
                */
                ...
                for ($i = 1; $i < $row_count; ++$i) {
                    ...
                    if ($qty > 0) {
                        ...
                        $this->Constant_model->insertData('return_purchase_order_items', $ins_po_item_data);
                    // Update Product Inventory;
                    $ckInvResult = $this->db->query("SELECT * FROM inventory WHERE product_code = '$pcode' AND outlet_id = '$outlet_id' ");
                    $ckInvRows = $ckInvResult->num_rows();
                    if ($ckInvRows == 1) {
                        $ckInvData = $ckInvResult->result();
                        $ckInv_id = $ckInvData[0]->id;
                ...
                $this->session->set_flashdata('alert_msg', array('success', 'Create Purchase Order', "Successfully Created Purchase Order : $po_numb"));
                redirect(base_url().'purchase_order/return_po_view');
            }
            unset($ckPOResult);
            unset($ckPORows);
